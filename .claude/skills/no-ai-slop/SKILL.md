...
## Project note (RegainFlow)

`docs/DESIGN.md` "Voice" is this site's established register and wins where the two disagree:

- **Em dashes** are used deliberately across the site. Keep them sparse in new copy; do not
  repunctuate existing copy.
- **Claim-then-evidence** ("A pilot is not a result.") is DESIGN.md rule 1, not a binary
  contrast to strip. The manifesto and stage copy stay as written.

Everything else applies in full to new copy.
